...
# -------------------------------
# 2. Transformer Model Definition (Simplified for demo)
# -------------------------------
...
# -------------------------------
# 4. User Administration with Role Selection
# -------------------------------
class UserManager:
    def __init__(self):
    ...
    def register(self, username, password, user_info, role):
        """Register new user"""
        ...
        self.users[username] = {
            ...
            'info': user_info,
            'role': role,  # Use 'role' instead of 'user_type'
            'history': [],
...
# -------------------------------
# -------------------------------
# CLIP MODEL (ZERO-SHOT IMAGE + TEXT)
# -------------------------------
@st.cache_resource
def load_clip_model():
...
def recognize_food_with_clip(image, nutrition_df, top_k=5):
    """
    CLIP zero-shot food recognition using food names as text prompts
    """
    model, processor, device = load_clip_model()

    foods = nutrition_df["Food"].astype(str).tolist()
    prompts = [f"a photo of {food}" for food in foods]
    ...
    top_k = min(top_k, len(foods))
    top_probs, top_indices = torch.topk(probs, top_k)

    best_food = foods[top_indices[0].item()]
    best_confidence = float(top_probs[0].item())

    top_matches = [
        (foods[idx.item()], float(prob.item()))
        for idx, prob in zip(top_indices, top_probs)
    ]

    return best_food, best_confidence, top_matches
...
def main():
    ...
    # Sidebar - Updated with UMP logo and university name
    try:
        # First try to load ump_logo.png from local file
        st.sidebar.image("ump logo.png", width="container")
    except:
        # If local file not found, use a placeholder
        st.sidebar.markdown("### UMPSA Logo")
    
    # Add university name - CORRECTED LINE
    st.sidebar.markdown("### **University Malaysia Pahang Al-Sultan Abdullah - UMPSA**")
    st.sidebar.markdown("---")
    ...
    if st.session_state.current_user in user_manager.users:
        ...
        user_role = user_data.get('role', 'Not specified')
        ...
        if 'bmi' in user_info:
            ...
            try:
                bmi_float = float(bmi) if isinstance(bmi, (int, float, str)) and str(bmi).replace('.', '').isdigit() else 0
                category, color, _ = get_bmi_category(bmi_float)
                st.sidebar.markdown(f"**⚖️ BMI:** {bmi} ({category})")
            except:
                st.sidebar.markdown(f"**⚖️ BMI:** {bmi}")
...
# -------------------------------
# 10. Page Functions with Role Selection
# -------------------------------
def show_login_register(user_manager):
    ...
    with col1:
        ...
        with tab1:
            ...
            # Role selection for login (optional)
            role_login = st.radio(
                "Role",
                ["Student", "Staff"],
                horizontal=True,
        ...
        with tab2:
            ...
            role = st.selectbox(
                "Select Your Role",
                ["Student", "Staff"],
                help="Choose your role in the university",
                key="reg_role"
            )
            ...
            if st.button("Register", type="primary", key="reg_btn"):
                if not new_user or not new_pass:
                    ...
                    success, message = user_manager.register(new_user, new_pass, user_info, role)
                    if success:
                        st.success(message)
    ...
    with col2:
        ...
        try:
            ...
            # If logo.jpg is not found, display app name
            st.markdown("""
            <div style="text-align: center; padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                        border-radius: 10px; color: white;">
                <h1>🍎</h1>
                <h2>The Calorie Detective</h2>
                <p>UMPSA Nutrition System</p>
            </div>
            """, unsafe_allow_html=True)
        
        st.markdown("""
        ### 🍎 UMPSA - The Calorie Detective
        
        **Welcome to our AI-powered Nutrition System!**
        ...
        if input_method == "Upload Image":
            ...
            if uploaded_file:
                try:
                    ...
                    st.image(image, caption="Uploaded Food Image", width="container")
                    ...
                    with st.spinner("🤖 AI is analyzing food..."):
                        ...
                        food_name, confidence, top_matches = recognize_food_with_clip(image, nutrition_df)
                        ...
                        <div style="padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                            ...
                            <p>🤖 Based on Transformer CLIP model</p>
                        </div>
                        """, unsafe_allow_html=True)
            ...
            if camera_photo:
                try:
                    ...
                    st.image(image, caption="Captured Food Image", width="container")
                    ...
                    with st.spinner("🤖 AI is analyzing food..."):
                        ...
                        <div style="padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                            ...
                            <h1>{food_name}</h1>
                            <h3>Confidence: {confidence:.1%}</h3>
                            st.markdown("### 🔍 Top CLIP Matches")
for name, conf in top_matches:
    ...
    with col2:
        ...
        with st.expander("🤖 View AI Model Details"):
            ...
            **1. Transformer for Image Recognition**
            ```python
            Model: CLIP (Contrastive Language–Image Pretraining)
            Components:
            - Vision Encoder: Vision Transformer (ViT)
            - Patch Embedding
            - Transformer Encoder Blocks (Multi-Head Self-Attention, MLP, LayerNorm)
...
def show_food_recommendation(recommender, user_manager):
    ...
    # Get user's daily calorie needs if available
    daily_cal = None
    if all(key in user_info for key in ['gender', 'weight', 'height', 'age', 'activity']):
        daily_cal = calculate_daily_calories(
            user_info['gender'],
    ...
    if rec_type == "Weight Loss":
        ...
        if st.button("Generate Weight Loss Plan", type="primary"):
            ...
            if daily_cal:
                target_calories = max(1200, daily_cal - daily_calorie_deficit)
            else:
        ...
        with col1:
            if daily_cal:
                st.info(f"Your daily calorie need: **{daily_cal} cal**")
            ...
            protein_percent = st.slider(
                "Protein (%)",
                min_value=10,
                max_value=40,
                value=25
            )
            carb_percent = st.slider(
                "Carbohydrates (%)",
                min_value=30,
                max_value=60,
                value=50
            ...
            # Calculate grams
            protein_cal = target_calories * protein_percent / 100
            carb_cal = target_calories * carb_percent / 100
            fat_cal = target_calories * fat_percent / 100
        ...
        with col2:
            # Display nutrition pyramid
            st.subheader("📊 Nutrition Pyramid")
            
            # Create nutrition pyramid visualization
            fig = go.Figure()
            
            # Add pyramid layers
            layers = [
                ("Fats & Oils", 25, "#FF9800", "Moderate amount"),
                ("Protein Foods", 20, "#4CAF50", "2-3 servings"),
                ("Dairy", 15, "#2196F3", "2-3 servings"),
                ("Vegetables", 30, "#8BC34A", "3-5 servings"),
                ("Fruits", 20, "#FFC107", "2-4 servings"),
                ("Grains", 40, "#795548", "6-11 servings")
            ]
            
            # Display as horizontal bars
            for name, width, color, servings in layers:
                fig.add_trace(go.Bar(
                    x=[width],
                    y=[name],
                    orientation='h',
                    ...
                    text=[servings],
                    textposition='inside',
                    hovertemplate=f"<b>{name}</b><br>Recommended: {servings}"
                ))
            
            fig.update_layout(
                title="Daily Food Pyramid",
                xaxis_title="Relative Amount",
                yaxis_title="Food Group",
                barmode='stack',
                height=400,
                showlegend=False,
        ...
        if st.button("Generate Balanced Diet Plan", type="primary"):
            st.success("✅ Balanced nutrition plan generated!")
            
            # Display macronutrient breakdown
            st.subheader("📈 Daily Macronutrient Goals")
            
            col_m1, col_m2, col_m3, col_m4 = st.columns(4)
            with col_m1:
                st.metric("Total Calories", f"{int(target_calories)} cal")
            with col_m2:
                st.metric("Protein", f"{protein_g:.1f}g", f"{protein_percent}%")
            with col_m3:
                st.metric("Carbs", f"{carb_g:.1f}g", f"{carb_percent}%")
            with col_m4:
                st.metric("Fat", f"{fat_g:.1f}g", f"{fat_percent}%")
            
            # Balanced meal suggestions
            st.subheader("🥗 Balanced Food Suggestions")
            
            # Protein-rich foods
            with st.expander("💪 High-Protein Foods"):
                protein_foods = recommender.df.nlargest(10, 'Protein')
                for _, row in protein_foods.iterrows():
                    st.write(f"**{row['Food']}** - {row['Protein']}g protein, {int(row['Calories'])} cal")
            
            # Fiber-rich foods
            with st.expander("🌾 High-Fiber Foods"):
                # Check if 'Fiber' column exists
                if 'Fiber' in recommender.df.columns:
                    fiber_foods = recommender.df.nlargest(10, 'Fiber')
                    for _, row in fiber_foods.iterrows():
                        st.write(f"**{row['Food']}** - {row['Fiber']}g fiber, {int(row['Calories'])} cal")
                else:
                    # Estimate fiber based on category
                    high_fiber_categories = ['Vegetable', 'Fruit', 'Grain']
                    fiber_foods = recommender.df[recommender.df['Category'].isin(high_fiber_categories)].head(10)
                    for _, row in fiber_foods.iterrows():
                        st.write(f"**{row['Food']}** - Good source of fiber, {int(row['Calories'])} cal")
            
            # Healthy fats
            with st.expander("🥑 Healthy Fat Sources"):
                healthy_fat_foods = recommender.df[
                    (recommender.df['Category'].isin(['Fish', 'Nut'])) | 
                    (recommender.df['Food'].str.contains('Avocado|Olive|Nuts', case=False))
                ].head(10)
                if not healthy_fat_foods.empty:
                    for _, row in healthy_fat_foods.iterrows():
                        st.write(f"**{row['Food']}** - {row['Fat']}g fat, {int(row['Calories'])} cal")
                else:
                    st.write("Fish, Nuts, Avocados, Olive Oil")
            
            # Balanced meal examples
            st.subheader("🍽️ Sample Balanced Meals")
            
            meal_examples = [
                {
                    "name": "Mediterranean Bowl",
                    "calories": 450,
                    "description": "Quinoa, grilled chicken, roasted vegetables, olives, feta cheese",
                    "macros": "35g protein, 45g carbs, 15g fat"
                },
                {
                    "name": "Asian Salmon Salad",
                    "calories": 400,
                    "description": "Mixed greens, grilled salmon, edamame, sesame dressing",
                    "macros": "30g protein, 20g carbs, 25g fat"
                },
                {
                    "name": "Vegetarian Power Bowl",
                    "calories": 380,
                    "description": "Brown rice, black beans, avocado, corn, salsa",
                    "macros": "15g protein, 55g carbs, 12g fat"
                }
            ]
            
            for meal in meal_examples:
                with st.expander(f"{meal['name']} - {meal['calories']} cal"):
                    st.write(f"**Description:** {meal['description']}")
                    st.write(f"**Macronutrients:** {meal['macros']}")
        ...
        with col1:
            ...
            diet_type = st.selectbox(
                "Dietary Preference",
                ["Balanced", "High Protein", "Low Carb", "Vegetarian", "Mediterranean"]
            )
            ...
            exclude_foods = st.multiselect(
                "Exclude Foods (Optional)",
                ["Dairy", "Gluten", "Nuts", "Seafood", "Eggs", "Soy"],
                help="Select foods to exclude from the meal plan"
            )
        ...
        with col2:
            ...
            num_meals = st.slider("Number of meals per day", 3, 6, 4)
            
            if num_meals == 3:
                meal_names = ["Breakfast", "Lunch", "Dinner"]
                meal_percents = [30, 40, 30]
            elif num_meals == 4:
                meal_names = ["Breakfast", "Lunch", "Dinner", "Snack"]
                meal_percents = [25, 35, 30, 10]
            elif num_meals == 5:
                meal_names = ["Breakfast", "Snack", "Lunch", "Snack", "Dinner"]
                meal_percents = [25, 10, 25, 10, 30]
            else:
                meal_names = ["Breakfast", "Snack", "Lunch", "Snack", "Dinner", "Snack"]
                meal_percents = [25, 10, 20, 10, 25, 10]
            
            # Display meal distribution
            meal_data = []
            for i, (name, percent) in enumerate(zip(meal_names, meal_percents)):
                calories = calorie_target * percent / 100
                meal_data.append({
                    "Meal": name,
                    "Calories": calories,
                    "Percentage": percent
            ...
            fig = px.bar(
                ...
                y="Calories",
                color="Meal",
                text=[f"{p}%" for p in meal_percents],
                color_discrete_sequence=px.colors.qualitative.Set2
            )
            fig.update_layout(
                title=f"Daily Meal Distribution ({int(calorie_target)} cal)",
        ...
        if st.button("Generate Meal Plan", type="primary"):
            st.success(f"✅ {plan_duration} meal plan generated!")
            
            # Calculate number of days
            days = 1 if plan_duration == "1 Day" else 3 if plan_duration == "3 Days" else 7
            ...
            for day_idx, tab in enumerate(tabs):
                with tab:
                    st.subheader(f"🍽️ Day {day_idx+1} Meal Plan")
                    
                    # Adjust calorie target based on day (optional variation)
                    day_calories = calorie_target
                    if day_idx % 2 == 1:  # Alternate days slightly
                        day_calories = calorie_target * 0.95
                    
                    # Generate meals for the day
                    for meal_idx, (meal_name, percent) in enumerate(zip(meal_names, meal_percents)):
                        meal_calories = day_calories * percent / 100
                        
                        with st.expander(f"{meal_name} - {int(meal_calories)} cal"):
                            # Generate recommendations based on diet type
                            recommendations = recommender.recommend_by_calories(
                                meal_calories,
                                None,
                                limit=2
                            )
                            ...
                            if not recommendations.empty:
                                # Display options
                                option = st.radio(
                                    f"Choose your {meal_name} option:",
                                    [f"Option {i+1}: {row['Food']}" for i, (_, row) in enumerate(recommendations.iterrows())],
                                    key=f"day{day_idx}_meal{meal_idx}"
                                )
                                
                                # Show selected option details
                                selected_idx = int(option.split(":")[0].replace("Option ", "")) - 1
                                selected_row = recommendations.iloc[selected_idx]
                                
                                col_d1, col_d2 = st.columns(2)
                                with col_d1:
                                    st.write(f"**Food:** {selected_row['Food']}")
                                    st.write(f"**Category:** {selected_row['Category']}")
                                    st.write(f"**Calories:** {int(selected_row['Calories'])} cal")
                                with col_d2:
                                    st.write(f"**Protein:** {selected_row['Protein']}g")
                                    st.write(f"**Carbs:** {selected_row['Carbs']}g")
                                    st.write(f"**Fat:** {selected_row['Fat']}g")
                                
                                # Meal preparation tips
                                st.markdown("---")
                                st.write("**👨‍🍳 Preparation Tips:**")
                                
                                # Generate simple preparation tips based on food type
                                food_type = selected_row['Category'].lower()
                                if 'vegetable' in food_type or 'fruit' in food_type:
                                    st.write("- Wash thoroughly before use")
                                    st.write("- Can be eaten raw or lightly cooked")
                                    st.write("- Add herbs and spices for flavor")
                                elif 'meat' in food_type or 'fish' in food_type:
                                    st.write("- Cook to safe internal temperature")
                                    st.write("- Season with herbs and spices")
                                    st.write("- Pair with vegetables for balanced meal")
                                elif 'grain' in food_type:
                                    st.write("- Cook according to package instructions")
                                    st.write("- Rinse before cooking (if applicable)")
                                    st.write("- Can be prepared in bulk for multiple meals")
                            else:
                                st.write("No specific recommendations found. Try adjusting your preferences.")
                    
                    # Daily nutrition summary
                    st.markdown("---")
                    st.subheader("📊 Daily Nutrition Summary")
                    
                    # Calculate estimated totals (simplified)
                    estimated_protein = day_calories * 0.25 / 4  # 25% of calories from protein
                    estimated_carbs = day_calories * 0.50 / 4    # 50% of calories from carbs
                    estimated_fat = day_calories * 0.25 / 9      # 25% of calories from fat
                    
                    col_s1, col_s2, col_s3, col_s4 = st.columns(4)
                    with col_s1:
                        st.metric("Total Calories", f"{int(day_calories)} cal")
                    with col_s2:
                        st.metric("Protein", f"{estimated_protein:.0f}g", "25%")
                    with col_s3:
                        st.metric("Carbs", f"{estimated_carbs:.0f}g", "50%")
                    with col_s4:
                        st.metric("Fat", f"{estimated_fat:.0f}g", "25%")
            
            # Shopping list
            with st.expander("🛒 Generate Shopping List"):
                st.subheader("Weekly Shopping List")
                
                # Common ingredients by category
                categories = {
                    "Produce": ["Mixed greens", "Bell peppers", "Onions", "Garlic", "Tomatoes", "Carrots"],
                    "Protein": ["Chicken breast", "Salmon fillets", "Eggs", "Greek yogurt", "Tofu"],
                    "Grains": ["Brown rice", "Quinoa", "Whole wheat bread", "Oats"],
                    "Dairy": ["Milk", "Cheese", "Butter"],
                    "Pantry": ["Olive oil", "Spices", "Herbs", "Salt", "Pepper"]
                }
                ...
                for category, items in categories.items():
                    with st.expander(f"**{category}**"):
                        for item in items:
                            ...
                            with col_check:
                                checked = st.checkbox("", key=f"shop_{item}", value=True)
                            with col_item:
                                if checked:
                                    st.write(f"✓ {item}")
                                else:
                ...
                if st.button("📥 Download Shopping List"):
                    st.success("Shopping list generated! (Download functionality would be implemented here)")

def show_user_profile(user_manager):
    """Display user profile with role display"""
    ...
    with col1:
        ...
        <div style="text-align: center; padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
            ...
            <p>Last Login: {user_data.get('last_login', 'Unknown')[:10]}</p>
        </div>
        """, unsafe_allow_html=True)
