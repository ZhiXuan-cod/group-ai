...
# -------------------------------
# 4. User Administration - UPDATED with user_type
# -------------------------------
class UserManager:
    def __init__(self):
    ...
    def register(self, username, password, user_info, user_type):
        """Register new user"""
        ...
        self.users[username] = {
            ...
            'info': user_info,
            'user_type': user_type,  # Add user type
            'history': [],
...
def main():
    ...
    if st.session_state.current_user in user_manager.users:
        ...
        user_type = user_data.get('user_type', 'Not specified')
        
        st.sidebar.markdown(f"**👤 User:** {st.session_state.current_user}")
        st.sidebar.markdown(f"**👥 Type:** {user_type}")
...
# -------------------------------
# 10. Page Functions - UPDATED with user_type
# -------------------------------
def show_login_register(user_manager):
    ...
    with col1:
        ...
        with tab1:
            ...
            # User type selection for login (optional)
            user_type_login = st.radio(
                "User Type",
                ["Student", "Staff"],
                horizontal=True,
                key="login_type"
        ...
        with tab2:
            ...
            # User type selection for registration - ADDED
            user_type = st.radio(
                "Select User Type",
                ["Student", "Staff"],
                horizontal=True,
            ...
            if st.button("Register", type="primary", key="reg_btn"):
                if not new_user or not new_pass:
                    ...
                    success, message = user_manager.register(new_user, new_pass, user_info, user_type)
                    if success:
                        st.success(message)
    ...
    with col2:
        # Add image here instead of in page_config
...
def show_user_profile(user_manager):
    """Display user profile - UPDATED with user_type display"""
