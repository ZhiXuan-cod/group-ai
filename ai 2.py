...
def load_nutrition_data():
    ...
    try:
        ...
        # For demonstration, we'll create a sample dataframe with more items for better NLP matching
        data = {
...
def show_login_register(user_manager):
    ...
    with col2:
        ...
        st.image("https://static.vecteezy.com/system/resources/previews/011/401/422/non_2x/food-signal-online-food-ordering-logo-design-order-food-on-internet-restaurant-cafe-meals-delivery-online-free-vector.jpg", width=250)
        st.markdown("""
        ### 🍎 System Features
                    ...
                    if not search_results.empty:
                        ...
                        with col_s2:
                            if st.button("Vegetables", use_container_width=True):
                                st.rerun()
                        with col_s3:
                            if st.button("Protein", use_container_width=True):
                                st.rerun()
            else:
...
def show_health_analysis(user_manager):
    ...
    with st.form("health_form"):
        ...
        if st.form_submit_button("Update Health Data", type="primary"):
            # Calculate BMI
...
def show_food_recommendation(recommender, user_manager):
    ...
    if rec_type == "Weight Loss":
        ...
        if st.button("Find Matching Foods", type="primary"):
            ...
            for idx, row in matches.iterrows():
                with st.expander(f"{row['Food']} - {int(row['Calories'])} cal"):
                    ...
                    fig.update_traces(textposition='inside', textinfo='percent+label')  # FIXED: update_traces not update_trace
                    st.plotly_chart(fig, use_container_width=True)
        ...
        if st.button("Generate Balanced Meal", type="primary"):
        ...
        if st.button("Generate Muscle Gain Plan", type="primary"):
...
def show_user_profile(user_manager):
    ...
    with col2:
        if user_info:
            ...
            if 'bmi' in user_info:
                info_html += f"""
                BMI: {user_info['bmi']} ({user_info.get('bmi_category', '')})
                """
            info_html += "</div>"
